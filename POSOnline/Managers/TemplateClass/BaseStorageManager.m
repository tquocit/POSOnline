...
- (BOOL)restoreOfflineAllClassObjects:(Class)classObject directory:(NSString*)directory
{
    ...
    //Check exists
    NSError *error;
    NSArray *dirContents = [[NSFileManager defaultManager] contentsOfDirectoryAtPath:storageFile error:&error];
    if (error != nil || [dirContents count] <= 0)
        return NO;
    
    NSDictionary * rootObject = [NSKeyedUnarchiver unarchiveObjectWithFile:storageFile];
    return [self setDictionaryForAllClassObject:classObject data:rootObject];
}
