...
- (void)createTableWithOrientation:(EasyTableViewOrientation)orientation {
    ...
    if (self.isDisplayedPageControl) {
        self.pageControl = [[UIPageControl alloc] initWithFrame:CGRectMake(0, 95, self.bounds.size.width, 10)];
        self.pageControl.numberOfPages = self.numberOfCells;
...
- (void)changePage:(id)sender
{
    ...
    [self setContentOffset:CGPointMake(CGRectGetWidth(self.bounds) * page, 0)];
}
...
//set up page scrolling for easy table view
- (void)scrollViewWillEndDragging:(UIScrollView *)scrollView
                     withVelocity:(CGPoint)velocity
              targetContentOffset:(inout CGPoint *)targetContentOffset
{
    CGFloat fractionalPart = fmodf(targetContentOffset->x, _cellWidthOrHeight);
    BOOL roundDown = fractionalPart < (_cellWidthOrHeight / 2.f);
    if (roundDown) {
        targetContentOffset->x -= fractionalPart;
    } else {
        targetContentOffset->x += (_cellWidthOrHeight - fractionalPart);
    }
}
